...
const resultsResponse = [
  {
    lineitem:
      'http://localhost/moodle/mod/lti/services.php/2/lineitems/2/lineitem?type_id=1',
    results: [
      {
...
const tokenValid = {
  ...
  'https://purl.imsglobal.org/spec/lti-bos/claim/basicoutcomesservice':
    {
      lis_result_sourcedid:
        '{"data":{"instanceid":"5","userid":"22","typeid":"2","launchid":932474241},"hash":"86f641f363947a7c5e8b0007f612f5dda68c3b7a708b9be0812b7132df5b4075"}',
      lis_outcome_service_url:
        'https://alfa.educsaite.org/mod/lti/service.php',
    },
  given_name: 'Carlos',
  family_name: 'Vinícius',
  name: 'Carlos Vinícius',
  'https://purl.imsglobal.org/spec/lti/claim/ext': {
    user_username: 'cvmcosta10@hotmail.com',
    lms: 'moodle-2',
  },
  ...
  'https://purl.imsglobal.org/spec/lti/claim/launch_presentation': {
    ...
    return_url:
      'https://alfa.educsaite.org/mod/lti/return.php?course=3&launch_container=3&instanceid=5&sesskey=ejFUKvABeF',
  },
  ...
  'https://purl.imsglobal.org/spec/lti/claim/custom': {
    resource:
      'qualificacao_dos_processos_de_trabalho_na_ABS/EDUCSAITE/TRILHA_4/SAUDE_PESSOA_DEFICIENCIA_INFANCIA/EBOOK_2',
    system_setting_url:
      'https://alfa.educsaite.org/mod/lti/services.php/tool/2/custom',
    context_setting_url:
      'https://alfa.educsaite.org/mod/lti/services.php/CourseSection/3/bindings/tool/2/custom',
    link_setting_url:
      'https://alfa.educsaite.org/mod/lti/services.php/links/{link_id}/custom',
  },
...
describe('Testing Assignment and Grades Service', function () {
  ...
  it('Grades.getLineItems() expected to return valid lineitem list', async () => {
    ...
    token.nonce = encodeURIComponent(
      [...Array(25)].map(_ => ((Math.random() * 36) | 0).toString(36))
        .join``,
    );
        ...
        keys: [
          {
            ...
            e: 'AQAB',
            kid: '123456',
            n: 'VrJSr-xli8NfuAdk_Wem5BARmmW4BpJvXBx3MbFY_0grH9Cd7OxBwVYSwI4P4yhL27upa1_FCRwLi3raOPSJOkHEDvFwtyYZMvdYcpDYTv6JRVqbgEyZtHa-vjL1wBqqW75yPDRoyZdnA8MWrfyRUOak53ZVWHRKgBnP53oXm7M',
          },
        ],
      });
    ...
    nock('http://localhost/moodle')
      .get('/lineitems?type_id=2&tag=tag')
      .reply(200, lineItemsResponse);

    lti.onConnect(async (token, req, res) => {
      try {
        return res.send(
          await lti.Grade.getLineItems(token, { tag: 'tag' }),
        );
      } catch (err) {
        console.log(err);
        res.sendStatus(500);
    ...
    return chai
      ...
      .post(url)
      .type('json')
      .send({ id_token: payload, state: state })
      .set('Cookie', [
        'state' +
          state +
          '=s%3Ahttp%3A%2F%2Flocalhost%2Fmoodle.fsJogjTuxtbJwvJcuG4esveQAlih67sfEltuwRM6MX0; Path=/; HttpOnly;',
        'ltiaHR0cDovL2xvY2FsaG9zdC9tb29kbGVDbGllbnRJZDEy=s%3A2.ZezwPKtv3Uibp4A%2F6cN0UzbIQlhA%2BTAKvbtN%2FvgGaCI; Path=/; HttpOnly; SameSite=None',
      ])
      .then(res => {
  ...
  it('Grades.getLineItemById() expected to return valid lineitem', async () => {
    ...
    nock('http://localhost/moodle')
      .get('/mod/lti/services.php/2/lineitems/16/lineitem?type_id=1')
      .reply(200, lineItemsResponse[1]);

    lti.onConnect(async (token, req, res) => {
      try {
        return res.send(
          await lti.Grade.getLineItemById(
            token,
            'http://localhost/moodle/mod/lti/services.php/2/lineitems/16/lineitem?type_id=1',
          ),
        );
  ...
  it('Grades.updateLineItemById() expected to return valid updated lineitem', async () => {
    ...
    nock('http://localhost/moodle')
      .put('/mod/lti/services.php/2/lineitems/16/lineitem?type_id=1')
      .reply(200, lineItemsResponse[1]);

    lti.onConnect(async (token, req, res) => {
      try {
        return res.send(
          await lti.Grade.updateLineItemById(
            token,
            'http://localhost/moodle/mod/lti/services.php/2/lineitems/16/lineitem?type_id=1',
            lineItemsResponse[1],
          ),
  ...
  it('Grades.deleteLineItemById() expected to return valid updated lineitem', async () => {
    ...
    nock('http://localhost/moodle')
      .delete(
        '/mod/lti/services.php/2/lineitems/16/lineitem?type_id=1',
      )
      .reply(204);
    ...
    lti.onConnect(async (token, req, res) => {
      try {
        return res
          .status(204)
          .send(
            await lti.Grade.deleteLineItemById(
              token,
              'http://localhost/moodle/mod/lti/services.php/2/lineitems/16/lineitem?type_id=1',
            ),
          );
  ...
  it('Grades.createLineItem() expected to return newly created lineItem', async () => {
    ...
    nock('http://localhost/moodle')
      .post('/lineitems?type_id=2')
      .reply(200, newLineItem);
    ...
    lti.onConnect(async (token, req, res) => {
      try {
        ...
        return res.send(
          await lti.Grade.createLineItem(token, lineItem),
        );
      } catch (err) {
        console.log(err);
        res.sendStatus(500);
  ...
  it('Grades.deleteLineItem() expected to return sucess array', async () => {
    ...
    nock('http://localhost/moodle')
      .delete(
        '/mod/lti/services.php/2/lineitems/2/lineitem?type_id=1',
      )
      .reply(200);
    nock('http://localhost/moodle')
      .delete(
    ...
    lti.onConnect(async (token, req, res) => {
      try {
        return res.send(
          await lti.Grade.deleteLineItems(token, { tag: 'tag' }),
        );
      } catch (err) {
        console.log(err);
        res.sendStatus(500);
  ...
  it('Grades.scorePublish() expected to return sucess array', async () => {
    ...
    nock('http://localhost/moodle')
      .post(
        '/mod/lti/services.php/2/lineitems/2/lineitem/scores?type_id=1',
      )
      .reply(200);
    nock('http://localhost/moodle')
      .post(
        '/mod/lti/services.php/2/lineitems/16/lineitem/scores?type_id=1',
      )
      .reply(200);
    nock('http://localhost/moodle')
      .get('/lineitems?type_id=2&resource_link_id=5&tag=tag')
      .reply(200, lineItemsResponse);
    ...
    lti.onConnect(async (token, req, res) => {
      try {
        ...
        return res.send(
          await lti.Grade.scorePublish(token, grade, { tag: 'tag' }),
        );
      } catch (err) {
        console.log(err);
        res.sendStatus(500);
  ...
  it('Grades.result() expected to return results array', async () => {
    ...
    nock('http://localhost/moodle')
      .get(
        '/mod/lti/services.php/2/lineitems/2/lineitem/results?type_id=1',
      )
      .reply(200, [
        {
          id: 'http://localhost/moodle/mod/lti/services.php/2/lineitems/2/lineitem/results?type_id=1&user_id=2',
    ...
    nock('http://localhost/moodle')
      .get(
        '/mod/lti/services.php/2/lineitems/16/lineitem/results?type_id=1',
      )
      .reply(200, [
        {
          id: 'http://localhost/moodle/mod/lti/services.php/2/lineitems/16/lineitem/results?type_id=1&user_id=2',
    ...
    lti.onConnect(async (token, req, res) => {
      try {
        return res.send(
          await lti.Grade.result(token, { tag: 'tag' }),
        );
      } catch (err) {
        console.log(err);
        res.sendStatus(500);
