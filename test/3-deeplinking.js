...
const tokenValid = {
  ...
  'https://purl.imsglobal.org/spec/lti-dl/claim/deep_linking_settings':
    {
      ...
      deep_link_return_url:
        'https://ltiadvantagevalidator.imsglobal.org/ltitool/deeplinkresponse.html',
    },
  ...
  'https://purl.imsglobal.org/spec/lti-nrps/claim/namesroleservice': {
    context_memberships_url:
      'https://alfa.educsaite.org/mod/lti/services.php/CourseSection/3/bindings/2/memberships',
...
describe('Testing Deep Linking Service', function () {
  ...
  it('DeepLinking.createDeepLinkingForm expected to return valid deep linking form', async () => {
    ...
    lti.onDeepLinking(async (token, req, res) => {
      try {
        return res.send(
          await lti.DeepLinking.createDeepLinkingForm(token, item, {
            message: 'Successfully Registered',
          }),
        );
      } catch (err) {
    ...
    return chai
      ...
      .then(async res => {
        ...
        expect(
          res.text.includes(
            '<form id="ltijs_submit" style="display: none;" action="https://ltiadvantagevalidator.imsglobal.org/ltitool/deeplinkresponse.html" method="POST"><input type="hidden" name="JWT"',
          ),
        ).to.equal(true);
        expect(
          res.text.includes(
        ...
        const _payload = res.text
          .split(
            '<form id="ltijs_submit" style="display: none;" action="https://ltiadvantagevalidator.imsglobal.org/ltitool/deeplinkresponse.html" method="POST"><input type="hidden" name="JWT" value="',
          )[1]
          .split(
