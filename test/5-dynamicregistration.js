...
const dynamicRegistrationRequest = {
  openid_configuration:
    'http://localhost/moodledyn/openid_configuration',
  registration_token:
...
describe('Testing Dynamic registration Service', function () {
  ...
  it('Dynamic Registration endpoint expected to return valid closing message and register Platform', async () => {
    ...
    nock('http://localhost/moodledyn')
      .get('/openid_configuration')
      .reply(200, configurationInformation);

    nock('http://localhost/moodledyn')
      .post('/register')
      .reply(200, registrationResponse);
    ...
    return chai
      ...
      .then(async res => {
        ...
        const platform = await lti.getPlatform(
          'http://localhost/moodledyn',
          '123456',
        );
        expect(platform).to.be.instanceOf(Platform);
        await expect(platform.platformActive()).to.eventually.equal(
          false,
        );
        await expect(platform.platformUrl()).to.eventually.equal(
          configurationInformation.issuer,
        );
        const authConfig = await platform.platformAuthConfig();
        expect(authConfig.key).to.equal(
          configurationInformation.jwks_uri,
        );
        await expect(
          platform.platformAuthenticationEndpoint(),
        ).to.eventually.equal(
