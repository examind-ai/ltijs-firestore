...
const tokenValid = {
  ...
  'https://purl.imsglobal.org/spec/lti-ags/claim/endpoint': {
    scope: [Array],
    lineitems:
      'https://alfa.educsaite.org/mod/lti/services.php/3/lineitems?type_id=2',
    lineitem: '',
  },
...
describe('Testing Names and Roles Service', function () {
  ...
  it('NamesAndRoles.getMembers() expected to return valid member list', async () => {
    ...
    nock('http://localhost/moodle')
      .get('/members?role=Learner&limit=2')
      .reply(200, membersResult);

    lti.onConnect(async (token, req, res) => {
      try {
        return res.send(
          await lti.NamesAndRoles.getMembers(token, {
            role: 'Learner',
            limit: 2,
          }),
        );
  ...
  it('NamesAndRoles.getMembers() expected to return valid member list and include multiple pages', async () => {
    ...
    nock('http://localhost/moodle')
      .get('/members?role=Learner&limit=2')
      .reply(200, membersResult, {
        link: '<http://localhost/moodle/api/lti/courses/1/names_and_roles?since=623698163>; rel="differences",<http://localhost/moodle/api/lti/courses/1/names_and_roles?page=2&per_page=1>; rel="next",<http://localhost/moodle/api/lti/courses/1/names_and_roles?page=1&per_page=1>; rel="first",<http://localhost/moodle/api/lti/courses/1/names_and_roles?page=2&per_page=1>; rel="last"',
      });

    nock('http://localhost/moodle')
      .get('/api/lti/courses/1/names_and_roles?page=2&per_page=1')
      .reply(200, membersResult);
    ...
    return chai
      ...
      .then(res => {
        ...
        expect(result.differences).to.eql(
          'http://localhost/moodle/api/lti/courses/1/names_and_roles?since=623698163',
        );
        expect(result.members[0]).to.deep.include(
          membersResult.members[0],
        );
        expect(result.members[3]).to.deep.include(
          membersResult.members[1],
        );
      });
  });
      ...
      try {
        return res.send(
          await lti.NamesAndRoles.getMembers(token, {
            ...
            limit: 2,
            pages: 1,
            url: 'http://localhost/moodle/members/1',
          }),
        );
      } catch (err) {
        res.sendStatus(500);
